// NEON BOSS SHOOTER v1.3 STABLE BUILD
...
(() => {
  ...
  addEventListener("keydown", (e) => {
    ...
    if (k === "v" && state === "menu") {
      if (!save.corruptedUnlocked) alert("Corrupted Mode unlocks after defeating Worldbreaker at wave 50.");
      else {
  ...
  function spawnWave() {
    ...
    // Critical fix: chaos only starts after wave 50.
    if (wave > 50) {
      startChaosWave();
      return;
    }
    ...
    if (wave >= 4) for (let i = 0; i < Math.floor(wave / 3); i++) spawnEnemy("fast");
    if (wave >= 9) for (let i = 0; i < Math.floor(wave / 6); i++) spawnEnemy("tank");
    if (wave >= 5) {
  ...
  function startChaosWave() {
    ...
    for (let i = 0; i < count; i++) spawnEnemy(i % 3 === 0 ? "shooter" : i % 2 === 0 ? "fast" : "normal");
  }
  ...
  function spawnBoss(type, chaos, small) {
    if (countBosses() >= 4 && type !== "FINAL") return;
    const p = randomEdgePosition();
    let hp = 65 + wave * 17;
    ...
    if (type === "CHARGER") { hp *= 0.7; speed *= wave < 20 ? 1.0 : 1.35; color = "#ff5e3b"; }
    if (type === "DASHLINE") { hp *= 0.85; speed *= 1.1; color = "#ff9f43"; }
    if (type === "BULLET_HELL") { hp *= 0.8; speed *= 0.72; color = "#ffde59"; }
    if (type === "TANK_BOSS") { hp *= 1.65; speed *= 0.55; damage += 6; color = "#ff7bba"; }
    if (type === "SPLITTER") { hp *= 1.05; color = "#a77dff"; }
    if (type === "SUMMONER") { hp *= 0.95; speed *= 0.75; color = "#6cff7a"; }
    if (type === "SPIRAL") color = "#7dfcff";
    if (type === "SNIPER") { hp *= 0.85; speed *= 0.6; color = "#ffffff"; }
    if (type === "PULSER") { hp *= 1.15; speed *= 0.75; color = "#ff9f43"; }
    if (type === "MIRROR") color = "#b28cff";
    if (type === "HYBRID") { hp *= 1.3; damage += 4; color = "#ff3b93"; }
    if (type === "RAVAGER_1") { hp = 520 * 1.5; speed = 0; damage = 18; r = 70; color = "#5b6478"; }
    if (type === "RAVAGER_2") { hp = 900 * 1.5; speed = 86; damage = 30; r = 76; color = "#ff2f88"; }
    if (type === "FINAL") { hp = 2800 * 1.25; speed = 0; damage = 42; r = 125; color = "#ff2f88"; }
    if (type === "PAST_BOSS") { hp = 190 + wave * 8; speed = 80 + wave * 1.3; damage = 20; r = 40; color = "#b28cff"; }
    if (type === "RAVAGER_ECHO") { hp = 650 + wave * 12; speed = 90; damage = 28; r = 62; color = "#b28cff"; }
    if (type === "RAVAGER_BURNING") { hp = 700 + wave * 13; speed = 95; damage = 31; r = 64; color = "#ff5e3b"; }
    if (type === "RAVAGER_VOID") { hp = 720 + wave * 13; speed = 88; damage = 32; r = 66; color = "#6b3cff"; }
    if (type === "RAVAGER_IRON") { hp = 920 + wave * 15; speed = 70; damage = 34; r = 70; color = "#8cff9f"; }
    if (type === "RAVAGER_CROWNED") { hp = 1000 + wave * 16; speed = 95; damage = 36; r = 72; color = "#ffde59"; }
    if (chaos) { hp *= small ? 0.65 : 1.15; speed *= 1.08; damage += 4; }

    const boss = {
      x: type === "FINAL" || type === "RAVAGER_1" ? canvas.width / 2 : p.x,
      y: type === "FINAL" ? canvas.height * 0.22 : type === "RAVAGER_1" ? canvas.height / 2 : p.y,
      r, hp, maxHp: hp, speed, damage, boss: true, bossType: type, chaos, smallBoss: small, type: "boss", color,
      hit: 0, shootTimer: 1.2, specialTimer: 2.2, chargeTimer: player.dashCooldown * 2, summonTimer: 3,
      orbTimer: 6.5, laneTimer: 2.6, eyeTimer: 1.12, swordTimer: 5.2, slashTimer: 7.2,
      spin: 0, phase: 1, splitDone: false, dashPause: 0, dashVx: 0, dashVy: 0, downed: false
    };
  ...
  function shoot(now) {
    ...
    if (weaponName === "SHOTGUN") {
      ...
      for (let i = 0; i < pellets; i++) {
        ...
        createPlayerBullet(angle + spread, baseDmg * 0.75 * damageBoost * (shouldCrit(weaponName) ? 1.75 : 1), data.color, 4, 0.68, false, weaponName);
      }
  ...
  function handleWaveClear() {
    if (wave === 25 && wave25Stage === 0) { wave25Stage = 1; startRavagerAwakening(); return; }
    ...
    if (wave === 50 && finalStarted) { startFinalEndingCutscene(); return; }
    if (wave % 4 === 0 || wave > 50) bossClearReward(true);
    else { chooseWaveBuff(); wave++; spawnWave(); }
  }
  ...
  function updateBoss(e, dt, angle, mult) {
    ...
    if (e.bossType === "CHARGER") {
      if (wave >= 20) {
        ...
        if (e.chargeTimer <= 0) { e.chargeTimer = player.dashCooldown * 2; e.x += Math.cos(angle) * 55; e.y += Math.sin(angle) * 55; burst(e.x, e.y, e.color, 14, 220); }
      }
      e.x += Math.cos(angle) * bossSpeed * (wave < 20 ? 1.05 : 1.25) * dt * mult;
      e.y += Math.sin(angle) * bossSpeed * (wave < 20 ? 1.05 : 1.25) * dt * mult;
    } else if (e.bossType === "DASHLINE") updateDashlineBoss(e, dt, angle, mult);
    else if (e.bossType === "SNIPER") { e.x += Math.cos(angle) * bossSpeed * 0.35 * dt * mult; e.y += Math.sin(angle) * bossSpeed * 0.35 * dt * mult; }
    else { e.x += Math.cos(angle) * bossSpeed * dt * mult; e.y += Math.sin(angle) * bossSpeed * dt * mult; }
    ...
    if (e.bossType === "BULLET_HELL" || e.bossType === "SPIRAL" || e.bossType === "HYBRID") {
      if (e.shootTimer <= 0) {
        ...
        for (let i = 0; i < shots; i++) createEnemyBullet(e.x, e.y, e.spin + (i * Math.PI * 2) / shots, speed, 8, 18, e.color);
        playSound(120, 0.12, "sawtooth", 0.05);
      }
    } else if (e.bossType === "SNIPER") {
      if (e.shootTimer <= 0) { e.shootTimer = e.phase === 2 ? 0.9 : 1.4; createLineHazard(e.x, e.y, angle, "#ffffff", 0.75, 0.18, 28, 20); bossSfx("beam"); }
    } else {
      if (e.shootTimer <= 0) {
    ...
    if (e.bossType === "SUMMONER" && e.summonTimer <= 0) { e.summonTimer = 3.2; spawnEnemy("fast"); spawnEnemy("normal"); floatingText(e.x, e.y - 70, "SUMMON", "#6cff7a", 16); }
    if (e.bossType === "MIRROR" && e.summonTimer <= 0) { e.summonTimer = 4.2; spawnEnemy("mirror"); floatingText(e.x, e.y - 70, "MIRROR", "#b28cff", 16); }
    if (e.bossType === "PULSER" && e.specialTimer <= 0) { e.specialTimer = 3.5; createRingHazard(e.x, e.y, 40, 330, 16, "#ff9f43"); screenShake = Math.max(screenShake, 7); }
    if (e.bossType === "TANK_BOSS" && e.specialTimer <= 0) { e.specialTimer = 4.0; createRingHazard(e.x, e.y, 50, 280, 20, "#ff7bba"); spawnEnemy("tank"); }
    if (e.bossType === "SPLITTER" && e.hp < e.maxHp * 0.5 && !e.splitDone) { e.splitDone = true; spawnBoss("PAST_BOSS", false, true); spawnBoss("PAST_BOSS", false, true); floatingText(e.x, e.y - 80, "SPLIT", "#a77dff", 22); }
  }
  ...
  function updateHazards(dt) {
    for (const h of hazards) {
      ...
      if (h.type === "ring") {
        ...
        if (Math.abs(distance(player, h) - h.r) < 12 && h.timer > h.warning && canHitPlayer()) hurtPlayer(h.damage, "ring");
      }
      if (h.type === "quadrantBeam") {
        const active = h.timer > h.warning && h.timer < h.warning + h.duration;
        if (active && pointInQuadrant(player.x, player.y, h.quadrant) && canHitPlayer()) hurtPlayer(h.damage, "beam");
      }
      if (h.type === "laneStrike") {
        const active = h.timer > h.warning && h.timer < h.warning + h.duration;
        const playerLane = Math.floor(player.y / (canvas.height / 8));
        if (active && h.lanes.includes(playerLane) && canHitPlayer()) hurtPlayer(h.damage, "lane");
      }
      if (h.type === "line") {
        const active = h.timer > h.warning && h.timer < h.warning + h.duration;
        if (active) {
          ...
          if (perpendicular < h.width && canHitPlayer()) hurtPlayer(h.damage, "line");
        }
      }
      if (h.type === "sword") {
        const active = h.timer > h.warning && h.timer < h.warning + h.duration;
        if (active) {
          ...
          if (Math.abs(player.x - h.x) < h.width && Math.abs(player.y - sy) < 80 && canHitPlayer()) hurtPlayer(h.damage, "sword");
        }
      }
      if (h.type === "diagonal") {
        const active = h.timer > h.warning && h.timer < h.warning + h.duration;
        if (active) {
          ...
          if (Math.abs(diagonalValue) < 50 && canHitPlayer()) hurtPlayer(h.damage, "slash");
        }
      }
    }
    hazards = hazards.filter(h => h.type === "ring" ? h.timer < h.duration : h.timer < h.warning + h.duration);
  }
    ...
    for (const b of bullets) {
      ...
      for (const e of enemies) {
        if (e.bossType === "FINAL" && !e.downed) {
          if (distance(b, e) < b.r + e.r) { b.life = -1; burst(b.x, b.y, "#ff2f88", 3, 80); floatingText(b.x, b.y, "SHIELDED", "#ff2f88", 12); break; }
  ...
  function drawHazards() {
    for (const h of hazards) {
      if (h.type === "quadrantBeam") {
        const rect = getQuadrantRect(h.quadrant); const active = h.timer > h.warning;
        ctx.globalAlpha = active ? 0.42 : 0.18; ctx.fillStyle = h.color; ctx.fillRect(rect.x, rect.y, rect.w, rect.h);
        ctx.globalAlpha = active ? 0.85 : 0.35; ctx.fillStyle = "#ffffff";
        for (let i = 0; i < 8; i++) { const bx = rect.x + (i + 0.5) * (rect.w / 8); const fall = active ? rect.h : (h.timer / h.warning) * rect.h; ctx.fillRect(bx - 7, rect.y, 14, fall); }
        ctx.globalAlpha = 1;
      }
      if (h.type === "laneStrike") { const active = h.timer > h.warning; const laneHeight = canvas.height / 8; for (const lane of h.lanes) { ctx.globalAlpha = active ? 0.65 : 0.18; ctx.fillStyle = active ? "#ff002f" : "#ff1f4f"; ctx.fillRect(0, lane * laneHeight, canvas.width, laneHeight); } ctx.globalAlpha = 1; }
      if (h.type === "ring") { ctx.globalAlpha = 0.75; ctx.strokeStyle = h.color; ctx.lineWidth = 8; ctx.beginPath(); ctx.arc(h.x, h.y, h.r, 0, Math.PI * 2); ctx.stroke(); ctx.globalAlpha = 1; }
      if (h.type === "line") { const active = h.timer > h.warning; ctx.save(); ctx.translate(h.x, h.y); ctx.rotate(h.angle); ctx.globalAlpha = active ? 0.82 : 0.25; ctx.fillStyle = h.color; ctx.fillRect(0, -h.width / 2, canvas.width * 2, h.width); ctx.globalAlpha = active ? 0.95 : 0.35; ctx.fillStyle = active ? "#ffffff" : "#ff4f9f"; ctx.fillRect(0, -h.width / 8, canvas.width * 2, h.width / 4); ctx.restore(); ctx.globalAlpha = 1; }
      if (h.type === "sword") { const active = h.timer > h.warning; const sy = active ? ((h.timer - h.warning) / h.duration) * (canvas.height + 160) - 80 : -80; ctx.globalAlpha = active ? 0.95 : 0.35; ctx.fillStyle = h.color; ctx.fillRect(h.x - h.width / 2, active ? sy : 0, h.width, active ? 160 : canvas.height); ctx.globalAlpha = 1; }
      if (h.type === "diagonal") { ctx.save(); ctx.globalAlpha = h.timer > h.warning ? 0.8 : 0.25; ctx.strokeStyle = h.color; ctx.lineWidth = h.width; ctx.beginPath(); if (h.kind === "slashA") { ctx.moveTo(0, 0); ctx.lineTo(canvas.width, canvas.height); } else { ctx.moveTo(canvas.width, 0); ctx.lineTo(0, canvas.height); } ctx.stroke(); ctx.restore(); ctx.globalAlpha = 1; }
    }
  }
  ...
  function drawBossHealthBar() {
    ...
    if (boss.bossType === "FINAL" && !boss.downed) { ctx.font = "13px Arial"; ctx.fillStyle = "#c7d4ff"; ctx.fillText("SHIELDED: collect the orb to knock it down", canvas.width / 2, y + 43); }
  }
  ...
  function drawMenu() {
    ...
    ctx.fillStyle = save.corruptedMode ? "#ff2f88" : "#6b7280"; ctx.fillText(save.corruptedUnlocked ? "Press V: Corrupted Mode " + (save.corruptedMode ? "ON" : "OFF") : "Corrupted Mode: Locked until Worldbreaker is defeated", canvas.width / 2, canvas.height / 2 + 183);
  }
